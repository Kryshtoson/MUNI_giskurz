...
# Kartodiagramy vyjadrujici podil X, XX a XXX promenych v krajich ČR (smyslena data) -
...
kraje_data <- merge(kraje, kraje_data)

kraje_data_utm33 <- st_transform(kraje_data, 
                                 crs = 32633)                    # EPSG noveho koordinacniho systemu 
...
mapa_kartodiagram <- ggplot() +
                    ...
                    annotation_scale(location = "br",                      # vlozeni meritka do praveho dolniho rohu
                                     ...
                                     pad_y = unit (0.3,"in"),              # prostor mezi mapovym ramem a meritkem mapy
                                     width_hint = 0.22) +                  # podil plochy mapy, kterou bude meritko zabirat
                    scale_x_continuous("Zem. délka") +                     # popis osy x
                    scale_y_continuous("Zem. šířka")+                      # popis osy y
